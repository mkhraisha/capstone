...
public class GameController : MonoBehaviour {
    ...
    void initMap1() {
		...
		var startLocation = new Tile(0,0);
        ...
        for (int y = this.map1.GetLength(1) - 1; y >= 0; y--)
        {
            for (int x = 0; x < this.map1.GetLength(0); x++)
            {
                ...
                else if (path.Where(p => p.X == x && p.Y == y).Any())
                {
                    ...
                    Debug.Log("incomplete route found");
				Debug.Log("Map value at " + x + " " + y + " is "  + map1[x,y]);
            }
	...
	private void RunnerUpdate()
	{
		...
		Tile runnerp1TileStart = new Tile(0,0);
		Tile runnerp1TileEnd = new Tile (5,5);
		//Debug.Log (" map value updated at " + runnerp1TileStart.X + " " +  runnerp1TileStart.Y);

		searchParam1.StartLocation = runnerp1TileStart;
		searchParam1.EndLocaton = runnerp1TileEnd;
		searchParam1.Map = map1;
